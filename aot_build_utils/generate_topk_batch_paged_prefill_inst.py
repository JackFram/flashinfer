...
def get_cu_file_str(
    ...
    def get_insts(attention_variant, dtype_out):
        return "\n".join(
            [
                """template cudaError_t TopKBatchPrefillWithPagedKVCacheDispatched<{cta_tile_q}, {head_dim_qk}, {head_dim_vo}, {pos_encoding_mode}, {use_fp16_qk_reduction}, {mask_mode}, {attention_variant}, Params>(
    Params params,
    {dtype_out}* tmp_v,
...
using Params = TopKBatchPrefillPagedParams<{dtype_q}, {dtype_kv}, {dtype_out}, {idtype}>;
...
if __name__ == "__main__":
    pattern = (
        r"topk_batch_paged_prefill_head_qk_([0-9]+)_head_vo_([0-9]+)_posenc_([0-9]+)_"
