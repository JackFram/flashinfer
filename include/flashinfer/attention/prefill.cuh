...
template <typename KTraits, typename Params>
__global__ __launch_bounds__(KTraits::NUM_THREADS) void TopKBatchPrefillWithPagedKVCacheKernel(
    const __grid_constant__ Params params) {
  using DTypeQ = typename Params::DTypeQ;
#if (__CUDA_ARCH__ < 800)
  if constexpr (std::is_same_v<DTypeQ, nv_bfloat16>) {
...
cudaError_t TopKBatchPrefillWithPagedKVCacheDispatched(Params params,
  ...
  DISPATCH_NUM_MMA_KV(min(max_num_mma_kv_smem, max_num_mma_kv_reg), NUM_MMA_KV, {
    ...
    if constexpr (KTraits::IsInvalid()) {
      ...
      size_t smem_size = sizeof(typename KTraits::SharedStorage);
      auto kernel = TopKBatchPrefillWithPagedKVCacheKernel<KTraits, Params>;
      FLASHINFER_CUDA_CALL(
          cudaFuncSetAttribute(kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size));
      if (tmp_v == nullptr) {
        // do not partition kv
