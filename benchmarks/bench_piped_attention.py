...
def run_bench(
    ...
    wrapper_pipe = flashinfer.PipeBatchAttention(kv_layout="NHD")
    wrapper_pipe.plan(
        q_indptr.to(device),
        kv_indptr.to(device),
        torch.arange(num_blocks).int().to(device),
    ...
    ms_pipe = do_bench(lambda: wrapper_pipe.run(q, kv_data))
    o, lse = wrapper_pipe.run(q, kv_data)

    print(f"Elapsed time (old scheduler): {ms_old:.2f} ms")
    print(f"Elapsed time (mixed scheduler): {ms:.2f} ms")
    print(f"Elapsed time (piped scheduler): {ms_pipe:.2f} ms")
    ...
    bandwidth_piped_gb_s = total_bytes / (ms_pipe * 1e-3) / (1024**3)
    ...
    print(f"Memory bandwidth (piped scheduler): {bandwidth_piped_gb_s:.2f} GB/s\n")
