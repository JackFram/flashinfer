...
void TopKBatchPrefillWithPagedKVCacheRun(
    ...
    DLTensor* paged_kv_rope_pos_offset, DLTensor* o, DLTensor* qk_ptr, DLTensor* lse,
  ...
  DISPATCH_context(
      ...
      RaggedParams, PagedParams, TopKParams, [&] {
        ...
        DISPATCH_CTA_TILE_Q(plan_info.cta_tile_q, CTA_TILE_Q, {
          status = flashinfer::TopKBatchPrefillWithPagedKVCacheDispatched<
              CTA_TILE_Q, HEAD_DIM_QK, HEAD_DIM_VO, POS_ENCODING_MODE,
              /*use_fp16_qk_reduction=*/USE_FP16_QK_REDUCTION, MASK_MODE, AttentionVariant,
              TopKParams>(params, tmp_v, tmp_s, stream);
        });

        CHECK(status == cudaSuccess)
            << "TopKBatchPrefillWithPagedKVCache failed with error " << cudaGetErrorString(status);
        return true;
      });
}
